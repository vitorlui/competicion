...
parser.add_argument('--cuda', type=int, default=1, help='Cuda device index')
args = parser.parse_args()

# Variables globales
CHECKPOINT_DIR = "optuna_checkpoints"
CSV_OUTPUT_DIR = os.path.join("outputs", "optuna_logs")
...
def objective(trial):
    ...
    # Añadir info del trial a cada fila del log
    for row in metrics_log:
        row["Trial"] = trial.number
        row["LR"] = lr
        row["Weight Decay"] = weight_decay
        row["Batch Size"] = batch_size
        row["Final AUC"] = auc
        row["Final EER"] = eer

    # Guardar en CSV único para todos los trials
    csv_path = os.path.join(CSV_OUTPUT_DIR, f"optuna_training_log_{MODEL_NAME}.csv")
    ...
    try:
        ...
        is_best = auc > best_value
    except ValueError:
        # No hay trial válido aún en la base de datos
        is_best = True
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(CSV_OUTPUT_DIR, f"optuna_results_{MODEL_NAME}_{timestamp}.csv")
